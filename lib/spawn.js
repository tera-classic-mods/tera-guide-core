...
/** @type {number} */
const HIGHLIGHT_ITEM = 45411;

/** @type {number} */
const HIGHLIGHT_ITEM_BLUE = 45411;

/** @type {number} */
const HIGHLIGHT_ITEM_PURPLE = 45411;

/** @type {number} */
const HIGHLIGHT_ITEM_RED = 45411;

/** @type {number} */
const MARKER_ITEM = 45411;
...
class Util {
	...
	delay(handler, delay, ...args) {
		...
		if (!isNaN(delayNumber) && delayNumber > 0)
			return this.__mod.setTimeout(handler, delayNumber, ...args);
		else
			handler(...args);
	 ...
	 * @return {Object} Entity object of the binding point
	 */
	static applyDistance(loc, offsetDistance, offsetAngle) {
		const r = loc.w;
...
class Place {
	...
	render(duration) {
		...
		if (loc) {
			...
			loc = Util.applyDistance(loc, this.offsetDistance, 360 - this.offsetAngle);
