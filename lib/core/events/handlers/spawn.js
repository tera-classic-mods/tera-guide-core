...
module.exports.spawn = (deps, data, event, ent) => {
	...
	// In the classic, the ids are different, we replace them for compatibility
	// See "StrSheet_Collections" for details
	const compatReplacement = {
		"553": 533,
		"537": 543,
		"912": 579,
		"445": 455,
	...
	switch (subType) {
		...
		case "collection":
			deps.mod.send(...deps.proto.getData("S_SPAWN_COLLECTION"), {
				"gameId": itemUniqueId,
				"id": compatReplacement[event.id] || event.id,
				"amount": 1,
				"loc": loc,
				"w": loc.w,
				"extractor": false,
