...
class Commands {
	 ...
	 */
	constructor(deps) {
		const { mod, lang, speech, params, gui, zone, handlers } = deps;
		...
		mod.command.add(params.command, {
			...
			"$none": () => {
				...
				mod.command.message(`${cg}${lang.strings.module}: ${mod.settings.enabled ? lang.strings.enabled : lang.strings.disabled}`);
			...
			"debug": (arg1, arg2) => {
				...
				if (mod.settings.debug[arg1] === undefined)
					return mod.command.message(`${cr}Invalid sub command for debug mode: ${arg1}`);
			...
			"event": (arg1, arg2) => {
				...
				if (["load", "l"].includes(arg1)) {
					if (!arg2)
						return mod.command.message(`${cr}Debug (event load) invalid values: ${arg1}`);
				...
				if (["status", "s"].includes(arg1)) {
					...
					mod.command.message(`${cw}Guide ID: ${zone.guide.id}`);
					mod.command.message(`${cw}Guide name: ${zone.settings.name || "not defined"}`);
					// eslint-disable-next-line no-nested-ternary
					mod.command.message(`${cw}Guide type: ${zone.guide.type === SP ? "SP" : (zone.guide.type === ES ? "ES" : "standard")}`);
					...
					if (zone.guide.eventNames().length > 1) {
						...
						zone.guide.eventNames().forEach(key => {
							if (key !== "error")
								mod.command.message(`${cy}${key} (${zone.guide.listenerCount(key)})`);
						});
					} else
					...
					if (zone.guide.hooks.list.size !== 0) {
						...
						zone.guide.hooks.list.forEach(attr =>
							mod.command.message(`${cy}${attr.debug.name} [${attr.keys.toString()}]`)
						);
					} else
				...
				if (["trigger", "t"].includes(arg1) && arg2) {
					...
					if (zone.guide.listenerCount(arg2) === 0)
						return mod.command.message(`${cr}Debug (event trigger) invalid values: ${arg2}`);

					mod.command.message(`${cy}Triggering event: ${arg2}`);
				...
				try {
					mod.command.message(`${cy}Triggering event: ${arg1} | ${arg2}`);
					...
					mod.command.message(`${cr}Debug (event) invalid values: ${arg1} | ${arg2}`);
			...
			"spawnObject": (arg1) => {
				if (arg1) {
					if (mod.settings.dungeons[arg1]) {
						...
						mod.command.message(`${cg}${lang.strings.spawnObject} ${lang.strings.fordungeon} "${mod.settings.dungeons[arg1].name || arg1}": ${mod.settings.dungeons[arg1].spawnObject ? lang.strings.enabled : lang.strings.disabled}`);
					} else
						mod.command.message(`${cr}${lang.strings.dgnotfound}`);
				} else {
					mod.settings.spawnObject = !mod.settings.spawnObject;
					mod.command.message(`${cg}${lang.strings.spawnObject}: ${mod.settings.spawnObject ? lang.strings.enabled : lang.strings.disabled}`);
				}
			},
			...
			"verbose": (arg1) => {
				if (arg1) {
					if (mod.settings.dungeons[arg1]) {
						...
						mod.command.message(`${cg}${lang.strings.verbose} ${lang.strings.fordungeon} "${mod.settings.dungeons[arg1].name || arg1}": ${mod.settings.dungeons[arg1].verbose ? lang.strings.enabled : lang.strings.disabled}`);
					} else
						mod.command.message(`${cr}${lang.strings.dgnotfound}`);
				} else
			...
			"voice": () => {
				...
				mod.command.message(`${cg}${lang.strings.speaks}: ${mod.settings.speech.enabled ? lang.strings.enabled : lang.strings.disabled}`);
			},

			"stream": () => {
				mod.settings.stream = !mod.settings.stream;
				mod.command.message(`${cg}${lang.strings.stream}: ${mod.settings.stream ? lang.strings.enabled : lang.strings.disabled}`);
			},

			"lNotice": () => {
				mod.settings.lNotice = !mod.settings.lNotice;
				mod.command.message(`${cg}${lang.strings.lNotice}: ${mod.settings.lNotice ? lang.strings.enabled : lang.strings.disabled}`);
			},

			"gNotice": () => {
				mod.settings.gNotice = !mod.settings.gNotice;
				mod.command.message(`${cg}${lang.strings.gNotice}: ${mod.settings.gNotice ? lang.strings.enabled : lang.strings.disabled}`);
			},

			"dungeons": () => {
				Object.keys(mod.settings.dungeons).forEach(key => {
					if (!mod.settings.dungeons[key].name) return;
					mod.command.message(`${cw}${key} - ${mod.settings.dungeons[key].name}`);
				});
			},
